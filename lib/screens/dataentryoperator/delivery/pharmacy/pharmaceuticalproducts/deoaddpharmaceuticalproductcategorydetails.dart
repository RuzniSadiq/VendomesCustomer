...
class _AddPharmaceuticalProductCategoryDetailsState extends State<AddPharmaceuticalProductCategoryDetails> {
  ...
  _uploadpharmaceuticalproductcategoryData() async {
    ...
    try {
      ...
      var list = [pharmaceuticalproductcategoryNameController.text];
      await FirebaseFirestore.instance.

      collection('delivery')
          .doc("9WRNvPkoftSw4o2rHGUI").collection('pharmacys').doc(widget.pharmacyid).update({
        //add the fod category inside the mainfoodcategories array
        "mainpharmaceuticalcategories": FieldValue.arrayUnion(list)
      });
    } catch (e) {
      print(e);
    }
